...
### 8.3 PySpark Prompt Template

Used when `glue_target: PYSPARK` in the FSD front-matter. The tool generates this automatically via `tool.py prompt` — same command, different template selected based on `glue_target`.

```
# GHCP Generation Prompt — etl_sales_report_pyspark.py
# App: CRM | Target: PYSPARK | Pattern: GENERAL_ETL
# Paste this entire file into GHCP chat

## YOUR TASK
Generate a COMPLETE, PRODUCTION-READY AWS Glue PySpark job script.
Output ONLY the Python script. No explanation. No markdown code fences.
The output will be saved directly as etl_sales_report_pyspark.py

## STRICT ARCHITECTURAL RULES — DO NOT DEVIATE

### Job type
AWS Glue PySpark job (NOT Python Shell).
Use GlueContext, SparkSession, DynamicFrame where appropriate.
Python 3.9 compatible syntax throughout.

### Database connections — TWO SERVERS (IN_MIGRATION stage)
SOURCE: Azure SQL MI (read-only during migration)
  secret_name: secret/crm/source-azure-sql-mi
  Use: spark.read.jdbc() with mssql JDBC driver for all source reads
  JDBC URL pattern: jdbc:sqlserver://{host}:1433;databaseName={db};encrypt=true

TARGET: AWS RDS SQL Server (all writes)
  secret_name: secret/crm/target-rds-sql-server
  Use: spark.write.jdbc() for DataFrame writes to RDS
  Use: raw pyodbc connection (autocommit=True) for DDL (TRUNCATE, DROP, CREATE)
  Use: raw pyodbc connection (autocommit=False) for DML (INSERT, UPDATE, MERGE)

STORED PROCEDURES: execute on TARGET via pyodbc — NOT via Spark JDBC
  PySpark cannot call stored procedures. Use pyodbc get_dml_conn() for all SP EXEC calls.

NEVER hardcode server names, usernames, passwords, or S3 paths.
NEVER use pandas in a PySpark job — use Spark DataFrames throughout.
...
Build JDBC URL and connection properties dict from Secrets Manager.
Pass properties dict to spark.read.jdbc() and spark.write.jdbc().
JDBC properties must include:
  "driver": "com.microsoft.sqlserver.jdbc.SQLServerDriver"
  "user": secret["username"]
  "password": secret["password"]

### Memory and performance rules
- Use spark.read.jdbc() with numPartitions and partitionColumn for large tables (> 1M rows).
  partitionColumn must be a numeric column. Provide lowerBound and upperBound from FSD.
- Use df.cache() only when the same DataFrame is referenced more than twice.
- Avoid df.collect() — use df.write or aggregations instead.
- Broadcast small lookup tables: spark.broadcast(small_df) or use F.broadcast() in joins.
- Use df.repartition() before large writes to avoid small file problem.

### DDL rule (CRITICAL — same as Python Shell)
autocommit=True BEFORE any DDL (TRUNCATE, DROP, CREATE, sp_rename) via pyodbc.
autocommit=False BEFORE any DML via pyodbc.
Spark JDBC writes are autocommit by nature — do not wrap in pyodbc transactions.

### Idempotency rule
At script start: TRUNCATE or DROP/CREATE staging table via pyodbc DDL connection.
Ensures the Spark job can be re-run safely.

### Error handling
Wrap Spark operations and pyodbc operations in try/except separately.
On exception: send SNS alert and re-raise.
Do NOT call sc.stop() in except — Glue manages SparkContext lifecycle.

## BASE TEMPLATE TO FOLLOW
[tool inserts pyspark_base.py content here]

## JOB SPECIFICATION
[tool inserts full FSD content here — all sections §1 through §12]

## TRANSFORMATION INSTRUCTIONS
For each transformation in §4 of the FSD, apply the following mapping:
  source_read      → spark.read.jdbc(url, table, properties=jdbc_props, numPartitions=N)
  filter           → df.filter() or df.where()
  join             → df.join(other, on=keys, how="left/inner/right")
                     Broadcast hint if right side row_estimate < 100K:
                     df.join(F.broadcast(lookup_df), on=keys)
  derive           → df.withColumn("col", expression)
                     Use F.when().otherwise() for conditional expressions
                     Use F.col(), F.lit(), F.concat(), F.coalesce() for standard expressions
  aggregate        → df.groupBy(*keys).agg(F.sum(), F.count(), F.avg(), F.max(), F.min())
  window_function  → df.withColumn("col", F.rank().over(Window.partitionBy().orderBy()))
  sort             → df.orderBy() — only if semantically required (avoid for performance)
  union            → df.unionByName(other_df, allowMissingColumns=True)
  target_write     → df.write.jdbc(url, table, mode="append"/"overwrite", properties=props)
  sp_call          → pyodbc get_dml_conn() + cursor.execute("EXEC dbo.sp_name")

## GENERATE THE COMPLETE SCRIPT NOW.
```

### 8.4 PySpark Base Template (pyspark_base.py)

```python
#!/usr/bin/env python3
"""
AWS Glue PySpark Job — {APP_CODE} | {JOB_NAME}
Generated by: Migration Accelerator + GHCP | Stage: {MIGRATION_STAGE}
ARCHITECTURE: PySpark | spark.read/write.jdbc (data) | pyodbc (DDL + SP calls)
DO NOT hardcode credentials, bucket names, or server addresses.
DO NOT use pandas — use Spark DataFrames throughout.
"""
import sys
import boto3, json, logging
import pyodbc
...
def get_jdbc_props(secret: dict) -> dict:
    return {
        "driver":   "com.microsoft.sqlserver.jdbc.SQLServerDriver",
        "user":     secret["username"],
        "password": secret["password"],
...
def get_source_jdbc_url() -> tuple[str, dict]:
    """Returns (jdbc_url, props) for Azure SQL MI source reads."""
    s = get_secret(SOURCE_SECRET)
    url = f"jdbc:sqlserver://{s['host']}:1433;databaseName={SOURCE_DB};encrypt=true;trustServerCertificate=false"
    return url, get_jdbc_props(s)

def get_target_jdbc_url() -> tuple[str, dict]:
    """Returns (jdbc_url, props) for RDS SQL Server writes."""
    s = get_secret(TARGET_SECRET)
    url = f"jdbc:sqlserver://{s['host']}:1433;databaseName={TARGET_DB};encrypt=true;trustServerCertificate=false"
    return url, get_jdbc_props(s)

# ── pyodbc connections (DDL and SP calls only) ─────────────────
def get_ddl_conn():
    """Raw pyodbc — DDL only. autocommit=True is MANDATORY for DDL."""
    s = get_secret(TARGET_SECRET)
    conn_str = (
        f"DRIVER={{ODBC Driver 17 for SQL Server}};"
        f"SERVER={s['host']};DATABASE={TARGET_DB};"
        f"UID={s['username']};PWD={s['password']};Encrypt=yes;"
    )
    conn = pyodbc.connect(conn_str, autocommit=True)
    return conn

def get_dml_conn():
    """Raw pyodbc — SP calls and DML. autocommit=False for explicit transaction."""
    s = get_secret(TARGET_SECRET)
    conn_str = (
        f"DRIVER={{ODBC Driver 17 for SQL Server}};"
        f"SERVER={s['host']};DATABASE={TARGET_DB};"
...
def main():
    log.info(f"Starting {JOB_NAME}")
    source_url, source_props = get_source_jdbc_url()
    target_url, target_props = get_target_jdbc_url()
    # GHCP fills in all transformations here
...
**Key differences from Python Shell base template:**

| Aspect | Python Shell | PySpark |
|---|---|---|
| Data reads | `pandas.read_sql()` with `chunksize=50000` | `spark.read.jdbc()` with `numPartitions` for large tables |
| Data writes | `df.to_sql()` or `DataFrame.write` via sqlalchemy | `spark.write.jdbc()` with `mode="append"/"overwrite"` |
| DDL execution | `pymssql` + `autocommit(True)` | `pyodbc` + `autocommit=True` |
| SP calls | `pymssql` cursor | `pyodbc` cursor — Spark cannot call SPs |
| Memory model | 1 DPU = 16GB, chunked processing | G.1X (16GB/worker) or G.2X (32GB/worker), distributed |
| Job commit | Not needed | `job.commit()` required at end |
| Glue imports | `getResolvedOptions` only | `GlueContext`, `Job`, `SparkContext`, `getResolvedOptions` |
...
| QA-13 | PySpark jobs: job.commit() present | **BLOCK** (PYSPARK only) | `job.commit()` missing in PySpark job — Glue marks job as failed without it |
| QA-14 | PySpark jobs: no pandas imports | **BLOCK** (PYSPARK only) | `import pandas` in a PYSPARK job — use Spark DataFrames throughout |
| QA-15 | PySpark jobs: no df.collect() on large tables | WARNING (PYSPARK only) | `df.collect()` where row_count_estimate > 10K — memory risk |
...
python tool.py deploy --app crm --all --target-type PYSPARK
```

**Deployer configuration by target type:**

| Glue parameter | PYTHON_SHELL | PYSPARK |
|---|---|---|
| `Command.Name` | `pythonshell` | `glueetl` |
| `GlueVersion` | `3.0` | `4.0` |
| `WorkerType` | N/A — use `MaxCapacity` | `G.1X` (< 10M rows) or `G.2X` (> 50M rows) |
| `NumberOfWorkers` | N/A | 2 (< 10M), 4 (10M–50M), 6 (> 50M) |
| `MaxCapacity` | `1.0` (1 DPU = 16GB) | N/A — set WorkerType instead |
| `--additional-python-modules` | `pandas,sqlalchemy,pymssql` | `pyodbc` only |
| `--job-bookmark-option` | Not set | `job-bookmark-enable` if incremental |
...
### Session 9b — Generate a PySpark job using GHCP (end-to-end PySpark path)
**Sprint: Sprint 5**

> This session is the PySpark equivalent of sessions 3–7 combined into one GHCP conversation. It validates that the full PySpark path works end-to-end: approved PYSPARK FSD → prompt → GHCP generates PySpark script → QA checks pass → deploys to Glue.

**Open in VS Code before starting:**
- An approved FSD with `glue_target: PYSPARK` in `fsd/{app}/{job_name}.md`
- `config/base_templates/pyspark_base.py` (created in Session 9)
- The generated PySpark prompt file from `prompts/{app}/{job_name}_prompt.md`

**Step 1 — Generate the PySpark prompt:**
```bash
python tool.py prompt --app crm --job etl_sales_report
# Tool detects glue_target: PYSPARK → selects PySpark prompt template (§8.3)
# Output: prompts/crm/etl_sales_report_prompt.md
```

**Step 2 — Paste into GHCP chat:**

Open `prompts/crm/etl_sales_report_prompt.md` in VS Code and paste entire contents into GHCP chat. Then follow up with this additional context if the job has specific patterns:

```
[For jobs with large table reads — add after pasting the main prompt]
The source table dbo.SalesTransactions has approximately 15 million rows.
Use numPartitions=8 with partitionColumn="transaction_id" for parallel JDBC reads.
Set lowerBound=1 and upperBound based on MAX(transaction_id) queried first.

[For jobs with joins — add after pasting the main prompt]
The lookup table dbo.Products has approximately 50,000 rows.
Apply broadcast hint when joining with dbo.SalesTransactions:
  df_sales.join(F.broadcast(df_products), on="product_id", how="left")

[For jobs with window functions — add after pasting the main prompt]
The ranking logic uses RANK() OVER (PARTITION BY customer_id ORDER BY order_date DESC).
Use Window.partitionBy("customer_id").orderBy(F.col("order_date").desc())

[For jobs with SP calls mixed with Spark transforms — add after pasting the main prompt]
After the Spark transformation completes and data is written to staging:
Call dbo.sp_calculate_commissions via pyodbc get_dml_conn() — NOT via Spark JDBC.
SP execution must happen AFTER the Spark write completes, not during.
```

**Step 3 — Review GHCP output before saving:**

Before saving the generated script, check for these common GHCP mistakes on PySpark jobs:

```
✅ Check: GlueContext and Job are initialised before any Spark operations
✅ Check: job.commit() is called at the end of main() — not inside try block
✅ Check: spark.read.jdbc() uses properties dict (not inline user/password args)
✅ Check: SP calls use pyodbc cursor, not spark.sql() or JDBC
✅ Check: No pandas imports — PySpark jobs must use Spark DataFrames throughout
✅ Check: No df.collect() on large DataFrames — flag for replacement if seen
✅ Check: DDL operations (TRUNCATE) use pyodbc get_ddl_conn(), not Spark JDBC
✅ Check: autocommit=True is set before TRUNCATE in the pyodbc DDL connection
✅ Check: job.commit() is NOT inside a try/except — Glue requires it to run on success only
```

If GHCP used pandas anywhere: ask GHCP to replace pandas operations with PySpark equivalents before saving.

**Step 4 — Save and run QA:**
```bash
# Save GHCP output to:
generated/crm/etl_sales_report.py

# Run QA check
python tool.py deploy --app crm --job etl_sales_report --dry-run
# Dry-run validates QA and prereqs but does not deploy to AWS
```

**Step 5 — Deploy:**
```bash
python tool.py deploy --app crm --job etl_sales_report
# Deployer detects glue_target: PYSPARK from FSD front-matter
# Creates Glue job with WorkerType=G.1X (or G.2X per row_count_estimate)
# NumberOfWorkers set from FSD row_count_estimate:
#   < 10M rows  → G.1X, 2 workers
#   10M–50M     → G.1X, 4 workers
#   > 50M       → G.2X, 4 workers
```

**Acceptance test:**
PySpark job deploys successfully. Runs against real source (Azure SQL MI) and target (RDS SQL Server dev) in AWS Glue dev environment. Output row count matches source pipeline output. QA-08 check confirms no Python Shell-only imports.

**Files produced:** `generated/crm/etl_sales_report.py` | `deployed/crm/etl_sales_report.py`

**PySpark-specific deployer config** (what `tool.py deploy` sets for PYSPARK jobs):

| Glue parameter | Python Shell value | PySpark value |
|---|---|---|
| `Command.Name` | `pythonshell` | `glueetl` |
| `Command.PythonVersion` | `3` (maps to 3.9) | `3` |
| `WorkerType` | N/A (uses MaxCapacity) | `G.1X` or `G.2X` |
| `NumberOfWorkers` | N/A | 2–4 (from row_count_estimate) |
| `MaxCapacity` | `1.0` | N/A (use WorkerType instead) |
| `GlueVersion` | `3.0` | `4.0` |
| `--additional-python-modules` | `pandas,sqlalchemy,pymssql` | `pyodbc` (PySpark has Spark natively) |
